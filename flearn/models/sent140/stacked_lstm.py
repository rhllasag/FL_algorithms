...
class Model(object):
    ...
    def create_model(self, optimizer):
        features = tf.placeholder(tf.int32, [None, self.seq_len], name='features') # Seq 25
        ...
        stacked_lstm = rnn.MultiRNNCell(
            [rnn.BasicLSTMCell(self.n_hidden) for _ in range(2)])#two layers LSTM  # n_hidden 100
        outputs, _ = tf.nn.dynamic_rnn(stacked_lstm, x, dtype=tf.float32)
        fc1 = tf.layers.dense(inputs=outputs[:,-1,:], units=30)
        pred = tf.squeeze(tf.layers.dense(inputs=fc1, units=1)) # squeeze remove 1 of input or an specific axis
